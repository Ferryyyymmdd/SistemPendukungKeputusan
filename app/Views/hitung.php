...
<ul class="nav nav-tabs" id="custom-content-below-tab" role="tablist">
	<li class="nav-item">
		<a class="nav-link" id="cc-krta-tab" data-toggle="pill" href="#cc-krta" role="tab" aria-controls="cc-krta"
			aria-selected="true">Mengukur Konsistensi Kriteria</a>
	</li>
	<li class="nav-item">
		<a class="nav-link" id="cc-alt-tab" data-toggle="pill" href="#cc-alt" role="tab" aria-controls="cc-alt"
			aria-selected="true">Matriks Perbandingan Alternatif</a>
	</li>
	<li class="nav-item">
		<a class="nav-link active" id="cc-hsl-tab" data-toggle="pill" href="#cc-hsl" role="tab" aria-controls="cc-hsl"
			aria-selected="true">Hasil Akhir</a>
	</li>
</ul>
<div class="tab-content" id="cc-tabContent">
	<div class="tab-pane fade show" id="cc-krta" role="tabpanel" aria-labelledby="cc-krta-tab">
		...
		foreach ($krta as $r) {
			echo "<th>" . $r->nama_kriteria . " (K" . $r->id_kriteria . ")</th>";
			$tot[$r->id_kriteria] = 0;
		}
		;
		echo "</tr>";
		foreach ($krta as $r) {
			echo "<tr><td>" . $r->nama_kriteria . "</td>";
			foreach ($krta as $r2) {
		...
		foreach ($krta as $r) {
			echo "<th class='text-blue'>" . number_format($tot[$r->id_kriteria], 2) . "</th>";
		}
		;
		...
		foreach ($krta as $r) {
			echo "<th>K" . $r->id_kriteria . " (" . number_format($prts[$r->id_kriteria], 2) . ")</th>";
		}
		;
		echo "</tr>";
		foreach ($krta as $r) {
...
// $alt=$this->M_data->data("tbl_siswa")->result();
		...
		foreach ($krta as $r) {
			...
			echo "<tr><th><span title='Penjumlah tiap kolom yang bersangkutan'>Jumlah</span></th>";
			foreach ($alt as $r2) {
				echo "<th class='text-blue'>" . number_format($tot[$r2->id], 2) . "</th>";
			}
			;
			echo "</tr></table></div>";
	...
	<div class="tab-pane fade show active" id="cc-hsl" role="tabpanel" aria-labelledby="cc-hsl-tab">
		<div class='card card-blue card-outline'>
			...
			<div class='card-body'>
				<p>Setelah menemukan bobot dari masing-masing kriteria terhadap alternatif yang sudah ditentukan,
					langkah selanjutnya adalah mengalikan bobot prioritas kriteria dengan bobot dari masing-masing
					alternatif, kemudian hasil perkalian tersebut dijumlahkan perbaris. Sehingga didapatkan total
					...
					<div class="col-sm-12">
						<table class="table table-bordered table-striped" border="1" width='100%'>
							<tr>
								...
								$smpl = [];
								foreach ($krta as $r) {
									echo "<th width='15%'>K" . $r->id_kriteria . " (" . number_format($prts[$r->id_kriteria], 2) . ")</th>";
								}
								echo "<th></th></tr><tr>";
								foreach ($alt as $r) {
									echo "<th>" . $r->nama . "</th>";
					...
					<div class='col-sm-12'>
						<table class='table table-bordered table-striped' border="1" width='100%'>
							<tr>
								...
								<?php
								foreach ($krta as $r) {
									echo "<th width='15%'>K" . $r->id_kriteria . "</th>";
								}
								...
								foreach ($smpl as $k => $v) {
									...
									<div class='progress-group'>
										...
										<div class='progress progress-sm'>
											<div class='progress-bar bg-<?= $abg ?>'
												style='width: <?= number_format($v * 100, 2) ?>%'></div>
										</div>
									</div>
								<?php } ?>
							</div>
			...
			<div class="card-footer">
				<button type='button' onclick='cetak()' class='btn bg-gradient-blue'><i class='fa fa-print'></i> Cetak
